% lab20_ex_pulse_shaping: IQk -> IQ0(n) -> IQn
% This is the complete, runnable script for Exercise 20.12.
...
if(1) 
    ...
    % =====================================================================
    % Parameters to Modify for Exercise 20.12
    % =====================================================================
    ...
    % =====================================================================
    % Set do_disturb = 1 to enable.
    % The exercise asks to start with ideal (row 1) and then change
    % one by one to the disturbed values (row 2).
    
    % Ideal values (no disturbance):
    %SNR_dB = 160; chan_gain = 1; chan_phase = 0; carDf = 0; carDph = 0; ADCppm = 0;
    
    % Disturbed values (from Listing 20.4, line 1074):
...
% Requirement 4: Set do_disturb = 1 (for Exercise 20.12)
do_disturb = 1; 
...
if (do_updown) % Frequency UP and DOWN, channel and disturbances are in between
    ...
    if (do_figures)
        % --- Plot 1.5: Passband Spectrum (for Ex 20.10) ---
        figure;
        plot_psd(y, fs, 'TX Passband Spectrum (y[n]) - Freq-UP');
        title('TX Passband Spectrum (y[n]) - Freq-UP');
        fprintf('Plotting passband spectrum. This may take a moment...\n');
        pause(0.1);
    end
    ...
    % --- Apply Disturbances in Baseband (easier to model) ---
    IQnn = IQnn_raw;
    
    if(do_disturb)
        fprintf('Applying disturbances...\n');
        
        % 1. Add channel influence (attenuation and phase shift)
        % This simulates Figure 20.20, plot 3
        IQnn = IQnn * chan_gain * exp(j*chan_phase);
        
        % 2. Add complex AWGN
        % This simulates Figure 20.20, plot 2
        s_power = mean(abs(IQnn).^2);
        if s_power == 0, s_power = 1; end % Avoid div by zero if signal is silent
        noise_power = s_power / (10^(SNR_dB/10));
        noise = sqrt(noise_power/2) * (randn(size(IQnn)) + 1j*randn(size(IQnn)));
        IQnn = IQnn + noise;

        % 3. Add carrier frequency and phase offsets (Receiver Imperfection)
        % This simulates Figure 20.20, plots 4, 5, 6
        % ADCppm is modeled as a carrier frequency offset
        carDf_total = carDf + (ADCppm * 1e-6 * fcar / fs) / (1 + ADCppm * 1e-6);
        IQnn = IQnn .* exp(j*(2*pi*carDf_total*n + carDph));
        
        if(do_figures)
            figure;
            plot_psd(IQnn, fs, 'RX Baseband Spectrum + Disturbances');
            pause(0.1);
        end
    end

...
do_decim = 1;      % 0/1 signal decimation in the receiver
...
Mdecim = 12;        % decimation order: 1, 2, 3, 4, 6, 8, 12, 24
...
if (do_decim)
    % 1. Symulacja przesunięcia próbkowania ADC (ADC Sampling Time Offset)
    % Przesuwamy cały sygnał o ułamek próbki (np. 0.5 próbki)
    ADCdt = 0.5;   % Offset w próbkach (0.0 do 1.0)
    N_len = length(IQn);
    n_idx = 1:N_len;
    
    % Interpolacja sklejana (spline) do przesunięcia sygnału w czasie
    % Uwaga: interp1 działa na wektorach, IQn może być pionowy lub poziomy, 
    % upewnij się, że wymiary się zgadzają. Tu zakładamy poziomy (1,n).
    % Jeśli IQn jest kolumnowy, użyj IQn.'
    IQn = interp1((n_idx-1), IQn, n_idx + ADCdt, 'spline');
    
    % Usunięcie NaN powstałych na końcach po interpolacji
    IQn(isnan(IQn)) = 0; 

    % 2. Decymacja z przesunięciem względem symbolu (Symbol Time Offset)
    % Wybieramy moment startu próbkowania wewnątrz symbolu
    SYMdt = 0.5; % Offset wewnątrz symbolu (0.0 do 1.0), 0.5 to środek (teoretycznie najlepiej)
                 % Ale jeśli ADCdt przesunęło sygnał, to 0.5 może już nie być optymalne.
    
    % Obliczenie indeksu startowego
    start_idx = 1 + round(SYMdt * Mdecim);
    
    % Właściwa decymacja (resample w dół)
    IQn = IQn(start_idx : Mdecim : end);
    
    % Aktualizacja parametrów po decymacji
    M = Mdecim;
    K = K / M; % Nowa liczba próbek na symbol
    
    % Skalowanie filtra PSF (bo zmieniliśmy częstotliwość próbkowania)
    hpsf = hpsf(1:M:end); 
    
    % Wyświetlenie info
    fprintf('Decymacja włączona. M=%d. Nowe K=%d.\n', M, K);
else
    M = 1;
end
